...
export const getMessages = () => {
  return async (dispatch) => {
    ...
    let messages = await axios.get('http://localhost:27017/messages')
    dispatch({
...
export const sendMessage = (privateMessage) => {
  return async (dispatch) => {
    ...
    let message = await axios.post('http://localhost:27017/messages', privateMessage)
    dispatch({
...
export const deleteMsg = (msgId) => {
  return async (dispatch) => {
    ...
    let deletemsg = await axios.delete(`http://localhost:27017/messages/${msgId}`)
    dispatch({
