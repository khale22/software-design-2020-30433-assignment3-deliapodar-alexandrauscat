...
const isrental = (req, res, next) => {
  const isrental = req.profile && req.profile.rental
  if (!isrental) {
    return res.status('403').json({
      error: "User is not a rental"
    })
  }
  next()
}
...
const createCharge = (req, res, next) => {
  ...
  myStripe.tokens.create({
    ...
    stripeAccount: req.profile.stripe_rental.stripe_user_id,
  }).then((token) => {
